...
export default class GameDay extends Component {
  ...
  onMatchChange(data, index) {
    ...
    this.props.change([
      ...this.props.data.slice(0, this.state.day),
      {
        name: daydata.name,
        matches: daydata.matches
          .slice(0, index)
          .concat(data)
          .concat(daydata.matches.slice(index + 1, daydata.matches.length))
      },
    ...
    return (
      <div className="gameday">
        <div className="gameday-header">
          <button className="gameday-change" onClick={() => this.changeDay(-1)}>
            <svg
